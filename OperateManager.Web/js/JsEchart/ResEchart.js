...
function DrowEchart(classID,restype, starTime, endTime) {
    ...
    $.ajax({
        ...
        method: 'GET',
        async: false,
        data: { type: 1, restype: restype, classid: classID, starTime: starTime, endTime: endTime },
    ...
    //    method: 'GET',
    //    data: { classid: classID, starTime: starTime, endTime: endTime },
    //    async: false,
    //    success: function (result) {
    //        if (result.Rcode == 1) {
    //            for (var j = 0; j < arrx.length; j++) {
    //                for (var i = 0; i < result.Rdata.length; i++) {
    //                    if (arrx[j] == result.Rdata[i].resname) {
    //                        seriesData[j]= result.Rdata[i].num
